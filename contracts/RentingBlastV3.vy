...
@dev This contract is the single user-facing contract for each Renting Market. It does not hold any NFTs, although it holds the rentals values and the protocol fees (payment tokens). It also manages the creation of vaults (as minimal proxies to the vault implementation) and implements the rental logic. The delegation functionality is implemented in the vaults.
...
BLAST_ADDRESS: constant(address) = 0x4300000000000000000000000000000000000002
...
def __init__(
    ...
    IBlast(BLAST_ADDRESS).configureGovernor(msg.sender)
...
def delegate_to_wallet(token_contexts: DynArray[TokenContext, 32], delegate: address):
    ...
    for token_context in token_contexts:
        ...
        assert msg.sender == self.id_to_owner[token_context.token_id], "not owner"
...
def deposit(token_ids: DynArray[uint256, 32], delegate: address):
    ...
    for token_id in token_ids:
        ...
        self._store_token_state(token_id, empty(Rental))
...
def start_rentals(token_contexts: DynArray[TokenContextAndListing, 32], delegate: address, signature_timestamp: uint256):
    ...
    for context in token_contexts:
        ...
        owner: address = self.id_to_owner[context.token_context.token_id]

        self._check_valid_listing(context.token_context.token_id, context.signed_listing, signature_timestamp, owner)
        ...
        self._consolidate_claims(
            context.token_context.token_id,
            self.id_to_owner[context.token_context.token_id],
            context.token_context.active_rental
        )
        ...
        self._store_token_state(context.token_context.token_id, new_rental)
...
def close_rentals(token_contexts: DynArray[TokenContext, 32]):
    ...
    for token_context in token_contexts:
        ...
        # clear active rental
        self._store_token_state(token_context.token_id, empty(Rental))

        # set unclaimed rewards
        self.unclaimed_rewards[token_context.active_rental.owner] += pro_rata_rental_amount - protocol_fee_amount
...
def extend_rentals(token_contexts: DynArray[TokenContextAndListing, 32], signature_timestamp: uint256):
    ...
    for context in token_contexts:
        ...
        # set unclaimed rewards
        self.unclaimed_rewards[owner] += pro_rata_rental_amount - protocol_fee_amount
...
def withdraw(token_contexts: DynArray[TokenContext, 32]):
    ...
    for token_context in token_contexts:
        assert self._is_context_valid(token_context), "invalid context"
        assert not self._is_rental_active(token_context.active_rental), "active rental"
        nft_owner: address = self.id_to_owner[token_context.token_id]
        assert msg.sender == nft_owner, "not owner"

        vault: IVault = self._get_vault(token_context.token_id)

        self._consolidate_claims(token_context.token_id, nft_owner, token_context.active_rental, False)
...
def claim(token_contexts: DynArray[TokenContext, 32]):
    ...
    for token_context in token_contexts:
        assert self._is_context_valid(token_context), "invalid context"
        nft_owner: address = self.id_to_owner[token_context.token_id]
        assert nft_owner == msg.sender, "not owner"

        result_active_rental: Rental = self._consolidate_claims(token_context.token_id, nft_owner, token_context.active_rental)
...
def claimable_rewards(nft_owner: address, token_contexts: DynArray[TokenContext, 32]) -> uint256:
    ...
    for context in token_contexts:
        ...
        assert self.id_to_owner[context.token_id] == nft_owner, "not owner"
...
@pure
@internal
def _state_hash(token_id: uint256, rental: Rental) -> bytes32:
    return keccak256(
        concat(
            convert(token_id, bytes32),
            rental.id,
...
def _is_context_valid(context: TokenContext) -> bool:
    ...
    return self.rental_states[context.token_id] == self._state_hash(context.token_id, context.active_rental)


@internal
def _store_token_state(token_id: uint256, rental: Rental):
    self.rental_states[token_id] = self._state_hash(token_id, rental)
...
def _consolidate_claims(token_id: uint256, nft_owner: address, active_rental: Rental, store_state: bool = True) -> Rental:
    if active_rental.amount == 0 or active_rental.expiration >= block.timestamp:
        ...
        if store_state:
            self._store_token_state(token_id, new_rental)
...
@internal
def _mint(tokens: DynArray[TokenAndWallet, 32]):
...
@internal
def _burn(tokens: DynArray[TokenAndWallet, 32]):
