...
def test_initial_state(
    vault_contract, renting_contract, nft_contract, ape_contract, delegation_registry_warm_contract
):
...
def test_create_vault_and_deposit_not_approved(contracts_config, renting_contract, nft_owner):
    token_id = 1
    with boa.reverts("not approved for token"):
        renting_contract.create_vault_and_deposit(token_id, sender=nft_owner)


def test_create_vault_and_deposit(contracts_config, renting_contract, nft_contract, nft_owner, vault_contract):
    token_id = 1
...
def test_change_listing_price(contracts_config, renting_contract, nft_contract, nft_owner):
    token_id = 1
    price = int(1e18)
    new_price = int(2e18)
...
def test_start_rental(contracts_config, renting_contract, nft_contract, ape_contract, nft_owner, renter):
    ...
    assert vault_contract.active_rental()[1:] == (renter, token_id, start_time, expiration, rental_amount)
...
def test_close_rental(contracts_config, renting_contract, nft_contract, ape_contract, nft_owner, renter):
    ...
    assert vault_contract.unclaimed_rewards() == int(Decimal(rental_amount) / Decimal(2))
