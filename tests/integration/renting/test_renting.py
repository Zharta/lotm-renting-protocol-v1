...
def test_create_vaults_and_deposit(
    ...
    renting_contract.create_vaults_and_deposit([token_id], price, 0, 0, True, sender=nft_owner)
...
def test_create_vaults_and_deposit_limits(contracts_config, renting_contract, nft_contract, nft_owner, vault_contract):
    ...
    renting_contract.create_vaults_and_deposit(token_ids, price, 0, 0, False, sender=nft_owner)
    event = get_last_event(renting_contract, "VaultsCreated")
...
def test_start_rental(contracts_config, renting_contract, nft_contract, ape_contract, nft_owner, renter, protocol_wallet):
    token_id = 1
    price = int(1e18)
    ...
    assert rental_log.protocol_fee == PROTOCOL_FEE
    assert rental_log.protocol_wallet == protocol_wallet


def test_close_rental(contracts_config, renting_contract, nft_contract, ape_contract, nft_owner, renter, protocol_wallet):
    token_id = 1
    price = int(1e18)
    start_time = boa.eval("block.timestamp")
    min_expiration = boa.eval("block.timestamp")
    ...
    protocol_fee_amount = real_rental_amount * PROTOCOL_FEE // 10000
...
def test_bulk_rentals_limits(contracts_config, renting_contract, nft_contract, ape_contract, nft_owner, renter):
    ...
    renting_contract.start_rentals([c.to_tuple() for c in token_contexts], duration, sender=renter)
    event = get_last_event(renting_contract, "RentalStarted")
...
@pytest.mark.profile
def test_bulk_rentals_limits_no_fee(contracts_config, renting_contract_no_fee, nft_contract, ape_contract, nft_owner, renter):
    bulk_size = 32
    token_ids = [token_id for token_id in range(bulk_size)]
    price = int(1e18)
    duration = 6
    ...
    renting_contract_no_fee.close_rentals([c.to_tuple() for c in token_contexts], sender=renter)
    event = get_last_event(renting_contract_no_fee, "RentalClosed")
    assert event.renter == renter
    assert event.nft_contract == nft_contract.address
    assert len(event.rentals) == bulk_size
    ...
    renting_contract_no_fee.claim([c.to_tuple() for c in token_contexts], sender=nft_owner)
...
def test_claim(contracts_config, renting_contract, nft_contract, ape_contract, nft_owner, renter):
    ...
    assert vault_contract.claimable_rewards(token_context.active_rental.to_tuple()) == rental_amount - protocol_fee_amount
    assert vault_contract.unclaimed_rewards() == 0
...
def test_withdraw_limits(contracts_config, renting_contract, nft_contract, ape_contract, nft_owner, renter):
    ...
    assert event.owner == nft_owner
    assert event.nft_contract == nft_contract.address
    assert event.total_rewards == (rental_amount - protocol_fee_amount) * len(token_ids)
    for idx, entry in enumerate(vaults.items()):
...
@pytest.mark.profile
def test_withdraw_limits_no_fee(contracts_config, renting_contract_no_fee, nft_contract, ape_contract, nft_owner, renter):
    token_ids = list(range(32))
    price = int(1e18)
    duration = 6
    rental_amount = duration * price
    ...
    renting_contract_no_fee.withdraw([c.to_tuple() for c in token_contexts], sender=nft_owner)
...
def test_deposit(contracts_config, renting_contract, nft_contract, ape_contract, nft_owner, renter):
    ...
    token_context.active_rental = Rental(
        ...
        rental_log.amount,
        rental_log.protocol_fee,
        rental_log.protocol_wallet,
    )
...
def test_deposit_limits(contracts_config, renting_contract, nft_contract, ape_contract, nft_owner, renter):
    ...
    renting_contract.deposit(token_ids, price, min_duration, max_duration, False, sender=nft_owner)
    event = get_last_event(renting_contract, "NftsDeposited")
...
def test_self_delegation(contracts_config, renting_contract, nft_contract, nft_owner, delegation_registry_warm_contract):
    ...
    renting_contract.cancel_listings_and_delegate_to_owner([token_context.to_tuple()], sender=nft_owner)
    assert delegation_registry_warm_contract.getHotWallet(vault_addr) == nft_owner
    delegation_registry_warm_contract.setHotWallet(ZERO_ADDRESS, 0, False, sender=vault_addr)

    # set_listings_and_delegate_to_owner creates self delegation
    token_context.listing = Listing(token_id, 0, 0, 0)
    renting_contract.set_listings_and_delegate_to_owner([token_context.to_tuple()], price, 0, 0, sender=nft_owner)
    assert delegation_registry_warm_contract.getHotWallet(vault_addr) == nft_owner
    delegation_registry_warm_contract.setHotWallet(ZERO_ADDRESS, 0, False, sender=vault_addr)

    # delegate_to_owner creates self delegation
    token_context.listing = Listing(token_id, price, 0, 0)
    renting_contract.delegate_to_owner([token_context.to_tuple()], sender=nft_owner)
    assert delegation_registry_warm_contract.getHotWallet(vault_addr) == nft_owner
